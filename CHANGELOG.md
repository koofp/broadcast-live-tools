...
## 2026-09-05 · 跨场记忆注入（预测记分卡滚动对账）

- **feat(session)**：`gen_session_summary` 新增 `{last_scorecard}` 占位——生成某场总结时，
  自动把上一场（按场次ID时间序配对，merge_archived 重建场不错配）的预测记分卡注入提示词，
  LLM 用本场事实逐条对账：已证伪/已验证滚动更新、未发生的继续跟踪、本场新预测追加表尾。
  0827 场实测：0826 的 6 条预测全部带本场证据更新（"本场仅3940附近，量能2.1万亿未达
  2.4万亿"等）。空态四分支齐备（首场/文件缺失/无记分卡/正常注入），`tests/test_session_memory.py`
  固化；verify.ps1 单测循环改自动发现 `tests/test_*.py`（新测试零注册成本）。
  至此博主思维蒸馏第一层（数据回路）+ 第二层第 1 项（跨场对账）落地；画像重蒸馏待积累。
