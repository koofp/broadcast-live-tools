...
### P1 · 近期（有触发条件再做）
- [ ] **场次聚合与整场总结**：按段开始时间间隔（≤35min 连续）把分段聚类为"场次"，生成场级元数据与整场二级总结（本场看点/时间线/名场面）；REPORT 与总结库加场次视图。边界注意：主播中途暂停 >5min 会被切成两场，可用弹幕 jsonl 活跃间隙或 blrec 开播/下播事件佐证
...
- 测试覆盖：批量转写/占位分支/批量总结/qa_check 均已真实执行 ✓
