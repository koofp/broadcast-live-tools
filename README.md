...
### P0 · 现在就值得做（低成本高价值）
- [ ] **全链路重测验证**（2026-08-24 已清空全部直播数据重置）：用 §5.98 合成视频法跑一遍 录制→转写→总结→场次→报告→告警，确认清空操作未破坏任何依赖；之后等 71003 首次真实开播做终验
...
- **全部直播数据已清空重置**（14.8GB 移入回收目录后彻底删除；元数据备份在 C 盘保留）
- 系统回到零点：容器 Up、71003 在配置内（开播即录）、面板/流水线/备份/通知全链路就绪
- 待办：全链路重测验证（P0 首项）→ 等首次真实开播终验
