...
### P0 · 现在就值得做（低成本高价值）
- [x] **全链路重测验证** ✅ 2026-08-24：`.\selftest.ps1` 六步断言全过（候选扫描/批量转写/占位分支/场次聚类/REPORT/自动清理）
- [x] **全链路离线终验** ✅ 2026-09-05：离线链路模拟测试 + 3 个实测 bug 修复（think 提额/tmp 竞争/run.lock 误删活锁）；剩余录制腿（真实开播 5-10 分钟全链）待用户开播，方案见 runbook §5.98.1
...
- [x] **总结库可见性修复 + 场次重算自动化** ✅ 2026-09-05：隐藏规则从房间级收敛为"场级覆盖段"精确隐藏（14323359 15 条段级整库隐身实锤）；场次重算挂进 process_all 每轮（sessions.json 永不再 stale）
- [ ] 用户决策项：房间 8139918 是否恢复自动录制（TI 已收官）；api_key.txt（OpenRouter legacy key）是否退役；1937830735 历史数据去留

### P1 · 近期（有触发条件再做）
- [x] **场次聚合与整场总结** ✅ 2026-08-24：`python session.py` 按 50 分钟间隔聚类分段为场次（派生视图幂等重算）；`--room X --summarize` 生成场级二级总结（一句话/时间线/要点/高光/金句 + 段级附录，指纹防过期）；REPORT 场次视图；总结库场次徽章；CLI 支持合并/拆分/命名纠错
- [ ] 多房间并行处理：触发条件=同时 ≥2 房间常态活跃且积压常超 10 段（当前单锁串行）
- [ ] 弹幕数据利用：总结附带高能弹幕时间戳（jsonl 已在录制，闲置中）
- [ ] cookie 时效巡检：每周自动跑 unlock-check 只读检测段
- [ ] 磁盘趋势曲线：仪表盘 7 天水位图（前置：需先给 status.ps1 加 append 式历史数据点，当前仅单快照覆盖写）
...
- **AI 供应商**：设置页可配（provider.json），当前 new-api 中继 + `DeepSeek-V4-Pro-0813-think`，
  21 个模型在线拉取；LLM 请求统一走 provider_config.fetch（代理失败直连回退+排障提示）
- **数据**：3 房间 27 段真实产物齐全（含 1937830735 的 2 个 flv 孤儿段；14323359×20 已聚 4 场/1790093449×2/1937830735×5）；
  最近一次真实录制 2026-09-01
- **全链路离线模拟测试 PASS**（selftest 六步 + 真实副本过新模型 + 异常路径，见 runbook §5.98.1）；
  **剩余终验**：录制腿（真实开播 5-10 分钟全链，待用户开播）
- **已知事项**：Clash 已退出（09-05）；LLM relay 曾被 TUN 分流阻断，排障见 runbook §5.7
